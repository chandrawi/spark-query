...
class MySQLTranslator implements ITranslator
{
    ...
    private static function valuesUpdate($query, array $values, int $count, bool $multiTableFlag)
    {
        $query->add(' SET ');
        ...
        foreach ($values as $value) {
            if ($value instanceof Value) {
                ...
                foreach ($vals as $i => $val) {
                    $query->add(self::QUOTE);
                    if ($multiTableFlag) {
                        $query->add($table. self::QUOTE. self::DOT. self::QUOTE);
                    }
                    $query->add($columns[$i]);
    ...
    {    
        ...
        if ($count) {
            ...
            foreach ($baseColumns as $i => $baseCol) {
                ...
                self::column($query, $baseCol, true);
    ...
    private static function where($query, array $whereClauses, $count, $multiTableFlag = false)
    {
        if ($count) {
            ...
            foreach ($whereClauses as $where) {
                ...
                $nestedConjunctive = self::conjunctive($nCon);
